...
function ServicesPage({ content }) {
  ...
  return (
    <AnimatedSection className="section-space min-h-screen bg-ivory pt-32 pb-24 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-accent/5 blur-[120px] rounded-full pointer-events-none"></div>
      <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-charcoal-200/5 blur-[100px] rounded-full pointer-events-none"></div>
          ...
          {categories.map((category) => (
            <div key={category.name} className="space-y-12">
              <div className="flex items-center gap-6">
                <h2 className="font-display text-3xl font-medium text-charcoal">{category.name}</h2>
